...
static int init_display(struct fbtft_par *par)
{
    ...
    /*
    	write_reg(par,0x0010,0x1690);
    	write_reg(par,0x0011,0x0227);
    	write_reg(par,0x0012,0x009D);
    	write_reg(par,0x0013,0x1900);
    	write_reg(par,0x0029,0x0025);
    */
    write_reg(par, 0x002B, 0x000D);
    //gamma
    write_reg(par, 0x0030, 0x0007);
    write_reg(par, 0x0031, 0x0303);
    write_reg(par, 0x0032, 0x0003); // 0006
    write_reg(par, 0x0035, 0x0206);
    write_reg(par, 0x0036, 0x0008);
    write_reg(par, 0x0037, 0x0406);
    write_reg(par, 0x0038, 0x0304); //0200
    write_reg(par, 0x0039, 0x0007);
    write_reg(par, 0x003C, 0x0602); // 0504
    write_reg(par, 0x003D, 0x0008);
    //ram
...
FBTFT_REGISTER_DRIVER(DRVNAME, "ilitek,ili9325a", &display);

MODULE_ALIAS("spi:" DRVNAME);
MODULE_ALIAS("platform:" DRVNAME);
MODULE_ALIAS("spi:ili9325a");
MODULE_ALIAS("platform:ili9325a");

MODULE_DESCRIPTION("FB driver for the ILI9325A LCD Controller");
